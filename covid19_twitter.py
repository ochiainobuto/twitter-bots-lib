...
for i in range(2, 110):
    try:
        tweet_data = api.search(q=serch_word, count=100, max_id=next_max_id-1)
        next_max_id = tweet_data[-1].id
        for tweet in tweet_data:
            tweet_text = tweet.text.replace('\n','')
            tweet_text = tweet_text.replace('\u3000','')
...
# for i in range(0,10):
#     print(i + 1, _c[i][0], _c[i][1])
tweet_word = 'Twitterでコロナを検索したときに多く含まれる名詞ベスト１０---'
for i in range(1,11):
    tweet_word = tweet_word + '■' +  str(i ) + '位: ' + str(_c[i][0]) + ':' + str(_c[i][1]) + '回 '
